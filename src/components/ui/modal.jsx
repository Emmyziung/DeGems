 ...
 const Modal = () => {
    ...
    return (
        ...
        <div className="modal-container bg-white">
           ...
           <button onClick={memberDisplay}><IoClose className="h-8 w-8 bg-white text-slate-600"/></button>
           </div>
          ...
          {people.map((p) => (
            <div
              key={p.name}
              className="rounded-xl  bg-card  p-4 flex flex-col items-center text-center"
            >
              ...
              <p className="mt-4 font-[650] text-foreground text-nowrap">{p.name}</p>
